...
class AgentV1:
    ...
    async def query_expansion_node(self, state: AgentState) -> AgentState:
        ...
        # Extract filters from the original query
        filters = await self.query_processor.extract_filters(state["current_message"])
        state["filters"] = filters

        # Filter expanded queries
        state["expanded_queries"] = [query for query in expanded_queries if self._query_meets_criteria(query, filters)]
        state["expansion_input_tokens"] = input_tokens
        state["expansion_output_tokens"] = output_tokens
        logger.info(f"Expanded queries: {state['expanded_queries']}")
        return state
    ...
    async def product_search_node(self, state: AgentState) -> AgentState:
        ...
        for query in [state["current_message"]] + state["expanded_queries"]:
            results = await self.weaviate_service.search_products(query, state["filters"], limit=5)
    ...
    async def result_reranking_node(self, state: AgentState) -> AgentState:
        ...
        reranked_products, input_tokens, output_tokens = await self.query_processor.rerank_products(
            state["current_message"], products_for_reranking, state["filters"], top_k=10, model=state["model_name"]
        )

        # Reorder the full Product objects based on the reranked products
        name_to_product = {p.name: p for p in state["search_results"]}
        state["final_results"] = [name_to_product[p["name"]] for p in reranked_products if p["name"] in name_to_product]
    ...
    async def response_generation_node(self, state: AgentState) -> AgentState:
        ...
        response, input_tokens, output_tokens = await self.openai_service.generate_response(
            user_message=user_message, system_message=system_message, temperature=0.7, model=state["model_name"]
        )
    ...
    def _query_meets_criteria(self, query: str, filters: Dict[str, Any]) -> bool:
        # Implement logic to check if the expanded query still meets the original criteria
        for key, value in filters.items():
            if isinstance(value, (int, float)):
                # For numeric values, check if the query contains a number greater than or equal to the filter value
                import re

                numbers = re.findall(r"\d+(?:\.\d+)?", query)
                if not any(float(num) >= value for num in numbers):
                    return False
            elif key.lower() not in query.lower():
                return False
        return True
            ...
            output = json.dumps(
                {
                    ...
                    "response_justification": f"There was an unexpected error in the system: {str(e)}",
                    "products": [],
