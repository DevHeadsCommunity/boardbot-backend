...
export const machine = setup({
  types: {
    context: {} as { webSocketRef: null; chatHistory: ChatMessage[] },
    events: {} as
      | {
  ...
  states: {
          ...
          states: {
            TextChatOpen: {
              ...
              on: {
                "user.typing": {
                  target:
                    "#chatActor.Connected.ChattingViaText.TextChatOpen.Typing",
                },
              },
              states: {
                initialRender: {
                  after: {
                    "500000": {
                      target: "#chatActor.Connected.ChattingViaText.Responding",
                ...
                Interactive: {
                  after: {
                    "500000": [
                      {
                        target:
                          "#chatActor.Connected.ChattingViaText.Responding",
                        actions: sendTo(
                          ({ context }) => context.webSocketRef!,
                          {
                            type: "webSocket.sendMessage",
            ...
            Responding: {
              ...
              on: {
                "webSocket.textMessageReceived": {
                  target:
                    "#chatActor.Connected.ChattingViaText.Responding.ReceivingResponse",
                  actions: assign({
                    chatHistory: ({
                      context,
                      event,
              ...
              states: {
                WaitingForFirstChunk: {
                  after: {
                    "5000": {
                      target:
                        "#chatActor.Connected.ChattingViaText.TextChatOpen",
                      actions: emit({
                        type: "notification",
                        data: {
                          type: "error",
                ...
                ReceivingResponse: {
                  always: [
                    {
                      target:
                        "#chatActor.Connected.ChattingViaText.TextChatOpen.Interactive",
                      guard: {
    ...
    Disconnected: {
      on: {
        "webSocket.connected": {
          target: "#chatActor.Connected.History",
        },
      },
    },
  },
